...
# 4. Obtención de la API Key desde los Secrets de Streamlit
api_key = st.secrets.get("GEMINI_API_KEY", "")

if not api_key:
    st.error("⚠️ No se encontró la API Key en los Secrets de la aplicación. Por favor, asegúrate de configurarla en Streamlit Cloud.")
    st.stop()
    ...
    if st.button("📋 Formular Diagnóstico Multiaxial"):
        if not texto_caso.strip():
            ...
            with st.spinner("Generando estructura de Diagnóstico Multiaxial..."):
                # Para la prueba pasa una cadena vacía o antecedentes adicionales si los hay
